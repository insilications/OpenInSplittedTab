...
import org.insilications.openinsplitted.codeInsight.navigation.actions.GotoDeclarationOrUsageHandler2Splitted
...
class ShowUsagesActionSplitted {
    companion object {
        ...
        private val createActionHandlerCachedInvoker: ((Project, SearchScope, SearchTarget) -> ShowUsagesActionHandler)? by lazy(LazyThreadSafetyMode.PUBLICATION) {
            try {
                ...
                { project, searchScope, target ->
                    // 2. Call the handle. Because `invokeWithArguments` is statically typed to
        ...
        fun showElementUsages(parameters: ShowUsagesParameters, actionHandler: ShowUsagesActionHandler) {
            val createActionHandlerInvoker = createActionHandlerCachedInvoker ?: return@underModalProgress null
            val showTargetUsagesActionHandler = try {
                createActionHandlerInvoker(project, searchScope, target)
            } catch (t: Throwable) {
                GotoDeclarationOrUsageHandler2Splitted.Companion.LOG.warn("Failed to invoke gotoDeclarationOrUsages", t)
                return null
            } ?: return null
